...
<head>
    ...
    <style>
        ...
        /* Zone du Timer d'Analyse Épique (1 minute) */
        .loader-box { display: none; background: #05020c; padding: 20px; border-radius: 12px; border: 1px solid #ff4757; margin-top: 20px; box-shadow: 0 0 15px rgba(255, 71, 87, 0.2); }
        .progress-bar { width: 100%; background: #110b22; height: 16px; border-radius: 8px; overflow: hidden; border: 1px solid rgba(255, 71, 87, 0.4); margin: 12px 0; }
        .progress-fill { width: 0%; height: 100%; background: linear-gradient(90deg, #ff4757, #ff6b81); transition: 1s linear; }
        .status-text { font-family: monospace; color: #ff6b81; font-size: 14px; min-height: 24px; }
...
<body>
    ...
    <div class="container">
        ...
        <div id="sec-ia-core" class="card" style="display:none;">
            ...
            <!-- Zone du Timer d'Attente de 1 minute intégrée directement ici -->
            <div class="loader-box" id="analysis-loader-box">
                <div class="status-text" id="loader-status">Démarrage des protocoles d'analyse...</div>
            ...
            <!-- Conteneur des résultats (masqué pendant le timer) -->
        ...
        <div id="sec-creer-page" class="card" style="display:none;">
            ...
            <textarea id="new-page-content" rows="4" placeholder="Contenu de la page..."></textarea>
    ...
    <script>
        ...
        // MOTEUR IA AVEC LE TIMER DE 1 MINUTE D'ANALYSE RIGIDE
        function launchIA() {
            ...
            if(!prompt) { alert("Écris quelque chose !"); return; }

            // Masquer les anciens résultats et le bouton, désactiver le bouton principal
            outputContainer.style.style = 'none';
            ...
            let timeLeft = 60;
            bar.style.width = "0%";
            status.innerText = "⚡ Initialisation des filtres neuronaux...";
            ...
            let interval = setInterval(() => {
                ...
                // Événement après 30 secondes (Il reste 30s) : Le secret arrive petit à petit !
                if (timeLeft === 30) {
                    status.innerText = "🔑 [SECRET ARRIVÉ 1/2] : Déchiffrement des paquets informatiques...";
                    status.style.color = "#00d2ff";
                }
                // Fin du timer de 1 minute (0s restante) -> Amène vers la page résultat
                else if (timeLeft <= 0) {
                    ...
                    // Génération du contenu final après l'attente
                    let textResult = "";
                    if (prompt.includes("comment coder") || prompt.includes("creer un truc") || prompt.includes("comment créer")) {
                        ...
                        textResult = "Salutations ! Je suis ton intelligence artificielle v1.2.5. Je suis pleinement opérationnelle et prête à t'assister pour générer du texte, du code ou des médias.";
                    } else {
                        textResult = "Analyse approfondie terminée. Votre requête nécessite l'activation de mes bases de données étendues. L'implémentation de cette idée demande une configuration modulaire et l'application rigoureuse de scripts d'automatisation.";
                    }
                    ...
                    if (mode === 'text' || mode === 'voice') {
                        if(mode === 'voice') {
                            ...
                            output.innerHTML = "💬 <b>Réponse Textuelle Experte :</b><br>" + textResult;
                        }
                        ...
                        output.innerHTML = `💻 <b>Bloc de Code Généré (${lang}) :</b><br><pre class="code-box">// Script Automatique ${lang}\\nfunction init() {\\n    // Système Prêt\\n}</pre>`;
                    }
                }
                else if (timeLeft > 30) {
                    status.innerText = "⚡ Analyse des mots-clés de la demande... [" + prompt.substring(0, 15) + "...]";
                }
                else if (timeLeft < 30 && timeLeft > 0) {
                    status.innerText = "🛸 Compilation des modules et injection des données expertes...";
                }
            }, 1000);
        }
